...
int main() {
	...
	{
		for (i = 5; i < G.handCount[thisPlayer]; i++)
		{
			printf("TEST: card = %d, expected = %d: ", testG.hand[thisPlayer][i], G.hand[thisPlayer][i]);
			if (testG.hand[thisPlayer][i] == G.hand[1][0] && testG.hand[thisPlayer][i] == G.hand[1][0])
			{
				printf("PASSED\n");
				passedCounter++;
			}
